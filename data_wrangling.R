...
sensors <- sensors %>% mutate(avg_interval = gsub("]|\\(", "", cut(.$ped_avg, breaks = c(100, 300, 600, 900, 1500, 2000)))) %>%
  separate(avg_interval, into = c("low_bound", "high_bound"), sep = ",", convert = TRUE)
